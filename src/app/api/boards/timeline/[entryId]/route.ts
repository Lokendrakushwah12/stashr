...
export async function PUT(
  ...
  try {
    ...
    if (updatedEntry?.boardId) {
      await models.Board.updateOne(
        { _id: updatedEntry.boardId },
    ...
    const entryFromDB = (await models.BoardTimelineEntry.findById(
      entryId,
    ).lean()) as (BoardTimelineEntryDocument & { images?: string[] }) | null;
    const entryObject = (
      updatedEntry.toObject ? updatedEntry.toObject() : updatedEntry
    ) as BoardTimelineEntryDocument & { images?: string[] };
    const updatedEntryAsDoc =
      updatedEntry as unknown as BoardTimelineEntryDocument & {
        images?: string[];
      };
