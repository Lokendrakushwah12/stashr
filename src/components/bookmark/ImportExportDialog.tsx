    ...
    const handleImport = async () => {
        ...
        try {
            ...
            if (result.imported > 0) {
                toast.success(`Imported ${result.imported} bookmarks into "${folderName.trim()}" folder`);
            }
            
            if (result.errors > 0) {
                toast.warning(`${result.errors} bookmarks were skipped (likely duplicates)`);
            }
                ...
                <div className="h-[calc(60vh+50px)] bg-background border border-border/70 rounded-xl flex flex-col">
                    <Tabs variant="bordered" defaultValue={activeTab} onValueChange={(val) => setActiveTab(val)} className="w-full h-full flex flex-col">
                        <TabsList className="grid w-full grid-cols-2 px-4 py-1">
                            ...
                            <TabsTrigger value="export" className="flex items-center justify-center gap-2 px-2 sm:px-3">
                                <Download className="h-4 w-4" />
                                Export
                            </TabsTrigger>
                        </TabsList>

                        <TabsContent value="import" className="flex-1 flex flex-col p-4 pt-2 pb-6">
                            ...
                            <ScrollArea className="flex-1 min-h-0 max-h-[50vh]">
                                <div className="space-y-4 pb-4 pr-4">
                                    ...
                                    >
                                        ...
                                        Supports JSON, HTML, CSV, and TXT formats. HTML files from browser exports are fully supported.
                                    </p>
                                                </div>
                                            </div>
                                        ) : (
                                    ...
                                    {importPreview && (
                                        <div className="space-y-3">
                                            ...
                                            <div className="max-h-40 w-full overflow-y-auto space-y-2 border rounded-lg p-3 bg-muted/20">
                                                {importPreview.bookmarks.slice(0, 5).map((bookmark, index: number) => (
                                                    <div key={index} className="text-sm p-2 bg-background rounded border">
                                                        <div className="font-medium truncate text-ellipsis max-w-lg">{bookmark.title}</div>
                                                        <div className="text-muted-foreground text-xs max-w-lg truncate">{bookmark.url}</div>
                                                    </div>
                                                ))}
                                            ...
                                            {showFolderInput && (
                                                <div className="space-y-2">
                                                    ...
                                                    />
                                                                                        <p className="text-xs text-muted-foreground">
                                        Bookmarks will be imported into a new folder with this name. Duplicate URLs will be automatically skipped.
                                    </p>
                                                </div>
                                            )}
                                        </div>
                            ...
                            {importPreview && showFolderInput && (
                                <div className="pt-4 border-t border-border bg-background">
                                    <Button
                                        ...
                                        disabled={isProcessing || bulkImport.isPending || !folderName.trim()}
                                        className="w-full"
                                    >
